...
int main(){
    ...
    // New plan:
    //
    // https://ics.uci.edu/~eppstein/163/lecture6b.pdf
    //
    // The bron-kerbosch algorithm:
    //
    // This algorithm makes a few notable optimizations on the naive apparoach to find
    // cliques in a graph. Clique:Subrgaph which is fully connected.
    //
    // The algorithm:
    // 1. The P set. Maintain a set of next possible graph nodes to visit.
    // 2. The X set. Maintain a visited set to eXclude from going next.
    //
    // if P is empty =>
    //      if X is empty => output clique
    // else:
    //      for v in P:
    //          N = neighbors of v
    //          RecursiveCall(
    ...
    //          move v from P to X
    //
    // Converting this to a linear algorithm:
    //
    // # Note: The graph used in this solution is a DAG, so there isnt much point maintaining a 
    // exclusion list
    //
    // Start with a stack containg a few fille adjancency list (P).
    // This will be a stack of P's
    // + Add this to the existing depth tracker 
    //
