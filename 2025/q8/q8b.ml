...
let in_chan = open_in "q8.txt" in
  ...
  let rec iter_n list circuits =
    ...
    if len_matched = 0 then
      ...
      iter_n t new_circuits
    else if len_matched = 1 then 
      (
      let head = List.hd matched_circuits in
      set_inserter head h;
      if (Hashtbl.length head) = n then
        let x1 = List.hd junct1 in
        let x2 = List.hd junct2 in
        x1 * x2
      else
      iter_n t circuits )
    else
      ...
      if (Hashtbl.length new_merged) = n then
        let x1 = List.hd junct1 in
        let x2 = List.hd junct2 in
        x1 * x2
      else
      ...
      iter_n t new_circuits )
    in
  let circuits = iter_n sorted_pairings [] in
  Printf.printf "%d\n" circuits;